...
int main(int argc, char **argv)
{
  ...
  command_pub = n.advertise<servo::servo_command>("servo_command", 100);
...
{
  Command cmd;
  servo::servo_command servoCommand;
  while(commandList.back().time < MILLIS * (tevent.current_real - tstart).toSec())
  {
    ...
    ROS_INFO("%d : executed (%s,%f) planned at time %d ", (int) (MILLIS * (tevent.current_real - tstart).toSec()), cmd.device.c_str(), cmd.value, cmd.time);
